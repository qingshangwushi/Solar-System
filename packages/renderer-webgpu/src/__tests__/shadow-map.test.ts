/**
 * WebGpuShadowMapPass 单元测试（任务 10 / E-07）。
 *
 * 验证：
...
describe('WebGpuShadowMapPass 构造与生命周期', () => {
  ...
  it('工厂函数创建实例', () => {
    const pass = createWebGpuShadowMapPass();
    expect(pass).toBeInstanceOf(WebGpuShadowMapPass);
  });
...
describe('WebGpuShadowMapPass prepare 契约', () => {
  ...
  it('prepare 计算光源投影矩阵（4×4，长度 16，正交投影）', () => {
    ...
    // 正交投影：m[0] = 2/(r-l)，m[5] = 2/(t-b)，m[10] = -2/(f-n)
...
describe('WebGpuShadowMapPass execute 契约', () => {
  const sampleBounds: BoundingBox = {
    min: { x: -50, y: -50, z: -50 },
    max: { x: 50, y: 50, z: 50 },
  };
  ...
  it('execute 按 resolution 创建深度纹理', () => {
    ...
    pass.execute(renderer, () => {});
    // 验证 createTexture 调用包含 1024x1024 的 depth32float
  ...
  it('execute 调用 beginPass（带深度附件）与 endPass', () => {
    ...
    pass.prepare(sampleDir, sampleBounds, DEFAULT_SHADOW_MAP_OPTIONS);
    pass.execute(renderer, () => {});
    // beginPass:0color:depth 表示带深度附件、无颜色附件
  ...
  it('execute 调用 sceneDrawFn 在 beginPass 与 endPass 之间', () => {
    ...
    // 验证 beginPass 在 sceneDrawFn 之前，endPass 在之后
...
describe('WebGpuShadowMapPass getShadowMapTexture 契约', () => {
...
describe('SHADOW_PCF_WGSL 着色器常量', () => {
  it('应是非空字符串', () => {
    expect(typeof SHADOW_PCF_WGSL).toBe('string');
    expect(SHADOW_PCF_WGSL.length).toBeGreaterThan(0);
  });

  it('应包含 sampleShadowPCF 函数定义', () => {
    expect(SHADOW_PCF_WGSL).toContain('sampleShadowPCF');
    expect(SHADOW_PCF_WGSL).toContain('fn sampleShadowPCF');
  });

  it('应包含 PCF 核循环（dy / dx 循环）', () => {
    expect(SHADOW_PCF_WGSL).toContain('for');
    expect(SHADOW_PCF_WGSL).toContain('dy');
    expect(SHADOW_PCF_WGSL).toContain('dx');
  });

  it('应支持 kernelSize 参数', () => {
    expect(SHADOW_PCF_WGSL).toContain('kernelSize');
  });

  it('应支持 bias 参数', () => {
    expect(SHADOW_PCF_WGSL).toContain('bias');
  });
});
